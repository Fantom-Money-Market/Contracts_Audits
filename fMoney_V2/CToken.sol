    ...
    TokenErrorReporter
{
    ...
    function initialize(
        ...
        require(err == NO_ERROR, "set comptroller fail");
        ...
        require(err == NO_ERROR, "set IR fail");
    ...
    function redeemFresh(
        ...
        require(
            ...
            "redeemTokensIn or redeemAmountIn must = 0"
        );
        ...
        internal
    {
        ...
        // Calculate minimum required cash based on total supply converted to underlying
        Exp memory exchangeRate = Exp({mantissa: exchangeRateStoredInternal()});
        uint256 totalUnderlying = mul_ScalarTruncate(exchangeRate, totalSupply);
        uint256 minRequiredCash = (totalUnderlying * utilBuffer) / 10000;
        
        // Check if borrow would leave less than the required buffer
        if (cashPrior < (borrowAmount + minRequiredCash)) {
    ...
    /**
     * @notice Updates the utilization buffer
     * @param newBuffer, the new % to use
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setUtilBuffer(uint256 newBuffer) external override returns (uint256) {
        require(msg.sender == admin, "!Admin");

        // Ensure buffer is not more than 20%
        require(newBuffer <= 2000, "> 20%");

        if(utilBuffer != newBuffer){
            emit UtilBufferSet(utilBuffer, newBuffer);
            utilBuffer = newBuffer;
        }
